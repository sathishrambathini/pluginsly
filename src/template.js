...
export default class Templates {
    ...
    static installedPluginsTemplate(pluginsData) {
        ...
        return `<div style='border: 1px solid #ddd;background:"${window._Plugins.widgetData.properties.background_check}";height: 100%;width: 100%;background-repeat: no-repeat;background-size: cover;overflow: auto;background-position: center;padding-top: 20px;padding-bottom: 20px;\'>              
            ...
            <div id="mainbox">
            {{#each data}}
                <div style="cursor:pointer;" class="card" data-toggle="tooltip" data-placement="top" title="{{name}}" onclick="window._Plugins.isPluginConfigured({{id}},'{{status}}','{{name}}','{{type}}')">
                <div style="height: 100%;"><img src="{{app_logo_url}}" style="height: 100%;width: 100%;border-radius: 50% !important;"></div>
                </div>
            {{/each}}
            </div>
            </div>`
                    ...
                    <div id="mainbox">
                        {{#each data}}
                            <div style="cursor:pointer;" class="card">
                                <div style="height: 70%;" data-toggle="tooltip" data-placement="top" title="{{name}}"><img src="{{app_logo_url}}"
                                style="height: 100%;width: 100%;border-radius: 50% !important;"></div>
                                <div style="text-align: center;margin-top: 22px;background:" #e6e2e27a";padding: 12px;margin-left:-10px;margin-right: -10px;color: #000;cursor: pointer;" onclick="window._Plugins.isPluginConfigured({{id}},'{{status}}','{{name}}','{{type}}')">{{status}}</div>
                            </div>
                        {{/each}}
                        </div>
                    </div>`
